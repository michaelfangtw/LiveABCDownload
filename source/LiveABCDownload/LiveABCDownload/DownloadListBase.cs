/*
********************************************************
功能:LiveABCDownloadBase 
說明:自動登入LiveABC取得下載清單之基礎類別
********************************************************
日期          作者       說明
2015.07.26    fij       初版 
2015.07.26    fij       CookieContainer保持登入
                        HtmlAgilityPack分析html內之link
********************************************************
*/
...
namespace LiveABCDownload
{
    ...
    /// LiveABCMp3List
    /// </summary>
    public class DownloadListBase
    {
        protected string DownloadUrlTemplate { get; set; }

        protected string DownloadYearMonth { get; set; }
        ...
        /// <returns></returns>
        protected string GetDownloadUrlTemplate(string key)
        {
        ...
        protected List<string> GetPageList(string downloadYearMonth, string downloadUrlTemplate, int pageCount)
        {
        ...
        protected IEnumerable<string> GetFileUrlList(string downloadYearMonth, string downloadUrlTemplate, int pageCount, string filetype = "zip")
        {
        ...
        protected StreamReader GetWebPageToStream(string url)
        {
        ...
        protected List<string> ExtractAllFileType(HtmlDocument htmlSnippet, string filetype = "zip")
        {
        ...
        public IEnumerable<string> Get(string yyyymm, int pageCount, string downloadUrlKey, string fileYype = "zip")
        {
            DownloadUrlTemplate = GetDownloadUrlTemplate(downloadUrlKey);
            DownloadYearMonth = yyyymm;
            PageCount = pageCount;//沒有分頁
            FileType = fileYype;
